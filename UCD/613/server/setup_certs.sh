...
keytool -export -keystore $keystore -storepass changeit -alias server \
    -file appdata/conf/server.crt 
cp appdata/conf/server.crt /ucddata/conf
...
for i in $(seq 1 20); do
    ...
    if [[ $crts ]]; then
        for crt in $crts; do
            ...
            echo keytool -importcert -keystore appdata/$keystore -storepass changeit \
                -alias $agent -file $crt -keypass changeit -noprompt
            status=$(keytool -importcert -keystore appdata/$keystore -storepass changeit \
